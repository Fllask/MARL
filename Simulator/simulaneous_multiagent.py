...
class AgentSimultanous(metaclass=abc.ABCMeta):
    def __init__(self, n_robots,rid, block_choices,use_wandb=False):
        super().__init__()
        self.rid = rid
...
class SACSparse(AgentSimultanous):
    def __init__(self,
        ...
        if self.env =='norot':
            ...
            self.n_actions = (1+len(block_choices))*(1+len(block_choices))*np.max(self.n_side_oriented_sup)*np.max(self.n_side_oriented)*6
    ...
    def choose_action(self,r_id,state,explore=True,mask=None):
        ...
        if self.exploration_strat == 'softmax':
            ...
            if np.random.rand() > self.eps:
                ...
                actionid = np.random.choice(ids)
        if self.env == 'norot':
            action,action_params = int2actsim_norot(actionid,state.graph.n_blocks,self.n_robots,self.n_side_oriented,self.n_side_oriented_sup,self.last_only,self.max_blocks)
        else:
            assert False, "Not implemented"
    ...
    def generate_mask(self,state):
        if self.env == 'norot':
            return generate_mask_norot(self.rid,state.grid, self.n_side_oriented,self.n_side_oriented_sup,self.last_only,self.max_blocks,self.n_robots,self.action_choice,state.type_id)
        else:
            assert False, "Not implemented"
def generate_mask_norot(rid,grid,n_side_b,n_side_sup,maxblocks,n_grounds,action_choice,typeblocks):
    mask = np.zeros((maxblocks+n_grounds,
                     n_side_sup.shape[0],
                     n_side_b.shape[0]+2,
                     np.max(n_side_sup),
                     np.max(n_side_b),
    ...
    if 'P' in action_choice:
        for i,n_side in enumerate(n_side_b):
            for j,typesup in enumerate(typeblocks):
                ...
                typesup +=1
                for k in range(6):
                    mask[j,typesup+1,i,:n_side_sup[typesup,k],:n_side[k],k]=True
    ...
    return mask.flatten()
def int2actsim_norot(actionid,n_blocks,n_side_b,n_side_sup,maxblocks,n_grounds):
