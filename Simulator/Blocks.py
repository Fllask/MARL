...
class Slide(Interface):
    ...
    def side_side_overlap(self,side_m,side_obst,dist_in,delta_dof):
        if ( abs(cross(side_m,side_obst)) > self.eps or abs(cross(side_m,dist_in)) > self.eps
            or abs(cross(side_m,delta_dof)) > self.eps):
            return False
        else:
            
            ax = np.argmax(np.abs(delta_dof))
            nsidem = side_m[ax]/delta_dof[ax]
            nsideo = side_obst[ax]/delta_dof[ax]
            nd = dist_in[ax]/delta_dof[ax]
            
            if abs(cross(side_obst,delta_dof))>self.eps:
                return False
                if cross(side_obst,delta_dof)>0:
                    if np.sign(nsidem)==1:
                        max_candidate = nd-nsideo
                        
            if np.sign(nsidem)==np.sign(nsideo):
                #intersection
                if np.sign(nsidem)==1:
                    max_candidate = nd-nsidem
                    min_candidate = nd+nsideo
                else:
                    min_candidate = nd-nsidem
                    max_candidate = nd+nsideo
                # if max_candidate > self.eps and max_candidate < 1-self.eps:
                self.max_list.append(max_candidate)
                self.n_obst.append(0)
                # if min_candidate > self.eps and min_candidate < 1-self.eps :
                self.min_list.append(min_candidate)
                self.n_obst.append(0)
            else:
                #slider
                ...
                if np.sign(nsidem)==1:
                    max_s = nd
                    min_s = nd-nsidem+nsideo
                else:
                    max_s = nd
                    min_s = nd-nsidem+nsideo
                
                self.slide_list.append([min_s,max_s])
        return True
    # def corner_corner_interaction(self,xm,xo,dof,alpha0m,alpha1m,alpha0o,alpha1o):
    #     deltax = xo-xm
        
    #     if (abs(cross(deltax,dof))>self.eps #or
    #         # abs(alpha0m-alpha1o)<self.eps or
    #         # abs(alpha1m-alpha1o)<self.eps or
    #         # abs(alpha0m-alpha0o)<self.eps or
    #         # abs(alpha1m-alpha0o)<self.eps or
    #         # abs(alpha0m+alpha1o)<self.eps or
    #         # abs(alpha1m+alpha1o)<self.eps or
    #         # abs(alpha0m+alpha0o)<self.eps or
    #         # abs(alpha1m+alpha0o)<self.eps
    #         ):
    #         print(f"{xo=}")
    #         return False
    #     else:
    ...
    #             phi = np.pi/2
    #         else:
    #             phi = np.arctan2(dof[1],dof[0])
    #         theta1o = (alpha1o-phi+np.pi)%(2*np.pi)-np.pi
    #         theta0o = (alpha0o-phi)%(2*np.pi)-np.pi
    #         print(theta0o/np.pi)
    #         print(theta1o/np.pi)
    #         if abs(theta0o) < self.eps or abs(abs(theta1o)-np.pi)<self.eps:
    #             #equivalent to the side/side interaction
    #             print('slide implicite')
    #             return False
    #         if (theta0o-self.eps)%(2*np.pi) < np.pi-self.eps and (theta1o-self.eps)%(2*np.pi)> np.pi+self.eps:
    #             print("max")
    #             self.hmax_list.append(xdof)
    #         elif abs(abs(theta0o)-np.pi)<self.eps or (theta1o > 0 and theta0o<0):
    #             self.hmin_list.append(xdof)
    #             print('min')
    #         else:
    #             print('slide')
    #             self.slide_list.append(xdof)
    #         return xdof
    #         # else:
    #         #     self.slide_list.append(xdof)
    #         #     return False
